...
old_soilStack <- stack(soilRasters)

#Trim soil layers to match training regions
crs(old_soilStack) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
...
envt_res <- raster("./Trimmed_env_layers/Aegopogon_alt.asc")
...
writeRaster(Aegopogon_soil2, filename = "./Layers/Trimmed_soil/Aaegopogon", 
...
writeRaster(Aspera_soil2, filename = "./Layers/Trimmed_soil/Aspera", 
            format = "ascii", bylayer = T, suffix=names(soilStack), NAFlag = "-9999", overwrite = T)

Barbigera_soil <- mask(soilStack, Barbigera) #Masking the raster stack
Barbigera_soil2 <- crop(Barbigera_soil, extent(NW)) 
writeRaster(Barbigera_soil2, filename = "./Layers/Trimmed_soil/Barbigera", 
            format = "ascii", bylayer = T, suffix=names(soilStack), NAFlag = "-9999", overwrite = T)
...
writeRaster(Melastomoides_soil2, filename = "./Layers/Trimmed_soil/Melastomoides", 
            format = "ascii", bylayer = T, suffix=names(soilStack), NAFlag = "-9999", overwrite = T)
...
writeRaster(Bipenicillata_soil2, filename = "./Layers/Trimmed_soil/Bipenicillata", 
            format = "ascii", bylayer = T, suffix=names(soilStack), NAFlag = "-9999", overwrite = T)
...
writeRaster(Verticillaris_soil2, filename = "./Layers/Trimmed_soil/Verticillaris", 
            format = "ascii", bylayer = T, suffix=names(soilStack), NAFlag = "-9999", overwrite = T)
