...
results_of_query2 <- tbl(conn, sql(query2)) %>% 
  collect()
results_of_query2
#Make lat long numeric
results_of_query2$latitude <- as.numeric(results_of_query2$latitude)
results_of_query2$longitude <- as.numeric(results_of_query2$longitude)

#Filter based on presence of lat long
filtered_results <- results_of_query2 %>%
  filter(!is.na(latitude), !is.na(longitude))

filtered_results_df <- as.data.frame(filtered_results)
filtered_results_df
#Remove incomplete records and unlikely coordinates
scrubbed_results <- coord_incomplete(filtered_results_df, lat = "latitude", lon = "longitude")
scrubbed_results <- coord_unlikely(scrubbed_results, lat = "latitude", lon = "longitude")
...
#Write data to file
write.csv(unique_results, "./Data/Occurrence_records_by_species/Taegopogon_unique_detailed.csv", row.names = FALSE)
